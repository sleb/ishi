# User Stories: `ishi daily`

`ishi daily` is sugar for `ishi new --daily` (see [new.md](new.md) story 013) — a
single command for today's note in the Inbox, without having to know or type
today's date.

## User Story 001 ✅

- **Summary:** Running `ishi daily` for the first time today creates the note
- **Depends on:** [new.md](new.md) Story 002 (named, non-interactive note creation), Story 007 (template rendering)

### Use Case

- **As a** Ishi user starting my day
- **I want to** run `ishi daily` and get a fresh note named for today
...
- **and Then:** Ishi prints the path it created, the same as any other non-interactive `ishi new` capture
- **and Then:** `$EDITOR` is not invoked — creation is non-interactive, same as `ishi new <filename>`

---

## User Story 002 ✅

- **Summary:** Running `ishi daily` again the same day opens the existing note instead of recreating it
- **Depends on:** Story 001 (note must already exist to be reopened)

### Use Case

- **As a** Ishi user returning to my daily note later in the day
- **I want to** run `ishi daily` again and pick up where I left off
...
- **When:** I run `ishi daily` again
- **Then:** Ishi opens the existing file directly in `$EDITOR`, unchanged from what I last saved — the template is not re-rendered and none of my earlier content is touched
- **and Then:** Ishi does not print a `Created ...` line, since no file was created
...
- **When:** I run `ishi daily`
- **Then:** Ishi reports an error that `$EDITOR` must be set to reopen an existing note, rather than silently doing nothing or recreating the file
...
- **As a** Ishi user relying on `ishi daily` for a consistent daily-note habit
...
- **When:** I run `ishi daily` on 2026-07-02
- **Then:** the file created is `0-Inbox/2026-07-02.md`

- **Scenario:** `ishi daily` takes no filename argument
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi daily some-name`
- **Then:** Ishi rejects the command with an error — `ishi daily` doesn't accept a filename argument, since the name is always today's date
