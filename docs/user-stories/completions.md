# User Stories: `ishi completions`

## User Story 001 ✅

- **Summary:** Get shell completions for `ishi` without writing them by hand
- **Depends on:** None

### Use Case

- **As a** Ishi user who wants tab-completion for `ishi` subcommands and flags
- **I want to** run `ishi completions <shell>`
...
- **When:** I run `ishi completions bash`
- **Then:** Ishi prints a bash completion script for `ishi` to stdout
- **and Then:** nothing is written to disk — I choose where to save it (e.g. `ishi completions bash > ~/.local/share/bash-completion/completions/ishi`)

- **Scenario:** Generating a zsh completion script
- **Given:** I am using zsh
- **When:** I run `ishi completions zsh`
- **Then:** Ishi prints a zsh completion script for `ishi` to stdout

- **Scenario:** Generating a fish completion script
- **Given:** I am using fish
- **When:** I run `ishi completions fish`
- **Then:** Ishi prints a fish completion script for `ishi` to stdout

- **Scenario:** Generating a PowerShell completion script
- **Given:** I am using PowerShell
- **When:** I run `ishi completions powershell`
- **Then:** Ishi prints a PowerShell completion script for `ishi` to stdout
...
- **As a** Ishi user who mistypes or guesses at a shell name
...
- **Given:** I run `ishi completions tcsh` (a shell Ishi doesn't generate completions for)
- **When:** the command runs
- **Then:** Ishi exits with an error naming the shells it does support (`bash`, `zsh`, `fish`, `powershell`)
- **and Then:** nothing is printed to stdout that could be mistaken for a completion script

- **Scenario:** Missing shell argument
- **Given:** I run `ishi completions` with no `<shell>` argument
- **When:** the command runs
- **Then:** Ishi exits with a usage error indicating `<shell>` is required, rather than guessing a default shell

---

## User Story 003 ✅

- **Summary:** Stay current with `ishi`'s commands as they change
...
- **As a** Ishi user who has installed a completion script
- **I want to** the script to reflect this installed version of `ishi`'s actual commands, subcommands, and flags
- **so that** completions don't drift out of sync with what `ishi` really accepts

### Acceptance Criteria

- **Scenario:** Completions cover all top-level commands
- **Given:** I run `ishi completions` for any supported shell
- **When:** Ishi generates the script
- **Then:** the script includes completions for every top-level command in `ishi`'s CLI definition (`init`, `new`, `daily`, `move`, `list`, `status`, `review`, `config`, `completions`), generated from that definition rather than a hand-maintained list
- **and Then:** running `ishi completions` again after a command or flag is added or removed (in a newer `ishi` version) reflects that change automatically, with no separate update to the completion script's source
...
- **As a** Ishi user running `ishi move`, `ishi archive`, or `ishi unarchive` on an item
...
- **so that** I don't have to guess or hand-type a name I might get subtly wrong — e.g. adding a `4-Archive/` prefix or a file extension that isn't part of the name `ishi` expects
...
- **When:** I type `ishi move ` and press tab (having installed the generated completion script for my shell)
...
- **When:** I type `ishi unarchive ` and press tab
- **Then:** the shell offers `Inbox/meeting-notes` as a completion — the `<OriginCategory>/<name>` form `ishi unarchive` expects, not the literal archive filesystem path
...
- **Given:** I am in a directory that is not a Ishi-initialized PARA system
- **When:** I request completions for an item-name argument
...
- **As a** Ishi user tab-completing an item name that happens to collide with another item's basename
- **I want to** be offered the qualified forms directly
- **so that** I never complete my way into the "ambiguous name" error move.md Story 006 introduced — tab-completion should only ever offer names `ishi move` will actually accept
...
- **When:** I type `ishi move ` (no further characters) and press tab
...
- **When:** I type `ishi move website` and press tab
...
- **Given:** `meeting-notes.md` exists in both `0-Inbox` and `3-Resources`
- **When:** I type `ishi move meeti` and press tab
...
- **Given:** `meeting-notes.md` exists in both `0-Inbox` and `3-Resources`
- **When:** I type `ishi move inbox/meeti` and press tab
