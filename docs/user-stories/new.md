...
- **As a** Ishi user who thinks of `new`'s flags as the one place category behavior lives
- **I want to** run `ishi new --daily` alongside `--project`/`--area`/`--resource`
- **so that** today's note is one more `new` variant instead of a separate command to remember

### Acceptance Criteria

- **Scenario:** `ishi new --daily` is equivalent to `ishi daily`
- **Given:** I am inside an initialized PARA system with the default `daily` template
- **When:** I run `ishi new --daily`
- **Then:** Ishi behaves exactly as `ishi daily` does (see [daily.md](daily.md)) — creating today's note non-interactively and printing the path if it doesn't exist yet, or opening it in `$EDITOR` if it does

- **Scenario:** `--daily` doesn't accept a filename
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new some-name --daily`
- **Then:** Ishi rejects the command with an error, since the daily note's name is always derived from the current date, not a supplied filename — unlike `--project`/`--area`/`--resource`, which require or accept one

- **Scenario:** `--daily` can't be combined with the other category flags
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new --daily --project`
- **Then:** Ishi rejects the command with an error, since `--daily`, `--project`, `--area`, and `--resource` are mutually exclusive
...
- **As a** Ishi user with a fleeting idea
- **I want to** run `ishi new` with no arguments and write directly in my editor
...
- **When:** I run `ishi new` with no arguments, and my editor opens pre-populated with the rendered `note` template, cursor positioned at the `{{cursor}}` mark on the title line
...
- **Then:** Ishi prompts `Create "website-improvement-ideas.md"?` with the inferred name pre-filled
- **and Then:** if I accept the prompt, the file is created in `0-Inbox` under that name, containing the frontmatter and heading exactly as I left them, and Ishi prints the path it created

- **Scenario:** Override the inferred filename
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new` with no arguments, fill in the pre-populated template, save, exit the editor, and am shown the inferred filename prompt
- **Then:** I can type a different filename instead of accepting the suggestion
- **and Then:** the file is created in `0-Inbox` under the name I typed, and Ishi prints the path it created

- **Scenario:** Heading found further down the note is still used
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new`, leave the pre-populated title line blank, write some body text, then add a heading line (e.g. `# Actual Title`) further down, save, and exit the editor
- **Then:** Ishi infers the title from that heading line, wherever it falls in the file — not just the line immediately after the frontmatter

- **Scenario:** No heading present falls back to the first line of content
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new`, delete the template's heading line entirely, write plain body text with no `#` heading anywhere, save, and exit the editor
- **Then:** Ishi infers the title verbatim from the first non-blank line after the frontmatter block, since no heading line was found

- **Scenario:** Leaving the template unmodified falls back to a timestamp
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new`, save the file exactly as it was pre-populated (no title typed in, no other edits), and exit the editor
- **Then:** Ishi prompts with a filename generated from the current timestamp instead of a note title, since the template alone has no title content to infer from

- **Scenario:** Emptying the file falls back to a timestamp
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new`, delete everything the editor was pre-populated with (including the frontmatter), save an empty file, and exit the editor
- **Then:** Ishi prompts with a filename generated from the current timestamp instead of a note title
...
- **As a** Ishi user who already knows what to call a note
- **I want to** run `ishi new <filename>` and skip the editor prompt
...
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new my-file`
- **Then:** a file named `my-file.md` is created in `0-Inbox` and Ishi prints the path it created
...
- **As a** Ishi user starting a new short-term effort
- **I want to** run `ishi new --project <filename>`
...
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new --project website-redesign`
- **Then:** a directory `1-Projects/website-redesign` is created containing an `index.md`, and Ishi prints the path to that `index.md`
...
- **As a** Ishi user taking on an ongoing responsibility
- **I want to** run `ishi new --area <filename>`
...
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new --area health`
- **Then:** a directory `2-Areas/health` is created containing an `index.md`, and Ishi prints the path to that `index.md`
...
- **As a** Ishi user saving a topic of ongoing interest
- **I want to** run `ishi new --resource <filename>`
...
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new --resource recipe-ideas`
- **Then:** a file named `recipe-ideas.md` is created in `3-Resources` and Ishi prints the path it created
...
- **As a** Ishi user creating notes throughout the day
...
- **Given:** I am inside an initialized PARA system
- **When:** I run `ishi new my-file` (or any `ishi new` variant) with a filename that has no extension
...
- **As a** Ishi user capturing a quick thought in `$EDITOR`
...
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new` with no arguments
...
- **Given:** I am inside an initialized PARA system with the default `note` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new`, type a title at the cursor, add body text below the frontmatter, save, and exit the editor
...
- **As a** Ishi user creating a note by name instead of through the editor
...
- **As a** Ishi user scaffolding a new project or area
...
- **As a** Ishi user starting a new project, area, or resource from a fleeting idea
- **I want to** run `ishi new --project` (or `--area`/`--resource`) with no filename
- **so that** I can write the content first in `$EDITOR` and let Ishi infer the name, without giving up the scaffolding those categories get
...
- **Given:** I am inside an initialized PARA system with the default `project` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new --project` with no filename, and my editor opens pre-populated with the rendered `project` template, cursor at the title line
- **and When:** I type `Website Redesign` at the cursor, save, and exit the editor
- **Then:** Ishi prompts `Create "website-redesign"?` with the inferred name pre-filled
- **and Then:** if I accept the prompt, a directory `1-Projects/website-redesign` is created containing an `index.md` with the frontmatter and my heading exactly as I left them, and Ishi prints the path to that `index.md`

- **Scenario:** Capture directly into a new area
- **Given:** I am inside an initialized PARA system with the default `area` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new --area` with no filename, and my editor opens pre-populated with the rendered `area` template
- **and When:** I fill in the title, save, exit the editor, and accept the inferred name
- **Then:** a directory `2-Areas/<inferred-name>` is created containing an `index.md` with the frontmatter and my content exactly as I left them, and Ishi prints the path to that `index.md`

- **Scenario:** Capture directly into a new resource
- **Given:** I am inside an initialized PARA system with the default `resource` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new --resource` with no filename, and my editor opens pre-populated with the rendered `resource` template
- **and When:** I fill in the title, save, exit the editor, and accept the inferred name
- **Then:** a file `3-Resources/<inferred-name>.md` is created with the frontmatter and my content exactly as I left them, and Ishi prints the path it created

- **Scenario:** Leaving the template unmodified still falls back to a timestamp
- **Given:** I am inside an initialized PARA system with the default `project` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new --project` with no filename, save the file exactly as it was pre-populated, and exit the editor
- **Then:** Ishi prompts with a name generated from the current timestamp instead of a note title, the same fallback used for a plain `ishi new` capture (per story 001)

- **Scenario:** Emptying the file still falls back to a timestamp
- **Given:** I am inside an initialized PARA system with the default `project` template
- **and Given:** my `$EDITOR` environment variable is set
- **When:** I run `ishi new --project` with no filename, delete everything the editor was pre-populated with, save an empty file, and exit the editor
- **Then:** Ishi prompts with a name generated from the current timestamp instead of a note title
...
- **As a** Ishi user with a custom template
...
- **As a** Ishi user practicing Zettelkasten-style note-taking
...
- **Given:** I am inside an initialized PARA system with a custom `note` template containing `{{uuid}}`
- **When:** I run `ishi new first-note` followed by `ishi new second-note`
