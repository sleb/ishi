...
Ishi is a command-line tool for managing a [PARA](https://fortelabs.com/para) system — a simple method for organizing your notes and files into four categories:
...
Ishi manages the directory structure and file bookkeeping so you can focus on capturing and organizing your notes:
...
Projects and areas are directories, not single files — real projects accumulate drafts, attachments, and other supporting material, and `index.md` is the entry point Ishi reads for a project or area's title and status. Resources and inbox captures are usually single notes, so they stay flat files.
...
```
git clone https://github.com/sleb/ishi.git
cd ishi
cargo install --path .
```

This installs an `ishi` binary.
...
$ cd my-para
$ ishi new meeting-notes
Created inbox/meeting-notes.md

$ ishi new --project website-redesign
...
| `config [init\|edit] [-g\|--global]`                      | View the effective config, or initialize/edit `.ishi.toml`; `-g` targets `~/.ishi.toml` instead of the local one                                                                                                                                                                                                                                    |
...
`--daily` creates (or opens) today's note in the Inbox — see [`ishi daily`](#daily), which is sugar for `ishi new --daily`. It doesn't take a `filename` (the name is always today's date) and can't be combined with `--project`/`--area`/`--resource`.
...
Creates (or opens) today's daily note in the Inbox, named for the current date. Sugar for `ishi new --daily`. The first run of the day creates the note non-interactively from the `daily` template and prints its path; running it again the same day opens the existing note in `$EDITOR` instead of recreating it.
...
```
ishi move <item> <inbox|project|area|resource|archive>
```
...
`<item>` also accepts an already-archived item's qualified `<OriginCategory>/<name>` form (as shown by `ishi list archive`); moving it to anything other than `archive` un-archives it — sugar for this is [`ishi unarchive`](#unarchive).

```
$ ishi move my-file.md project
Moved inbox/my-file.md to projects/my-file/index.md

$ ishi mv my-project archive
Moved projects/my-project to archive/projects/my-project

$ ishi mv Projects/my-project project
Moved archive/projects/my-project to projects/my-project
```

Moving a `project`/`area` directory to `inbox` or `resource` (unwrapping a directory back into a flat file) is not yet supported — `ishi move` rejects it with an error rather than guessing which file to keep.
...
Sugar for `ishi move <item> archive` — files an item away without having to name the destination category. It takes no category argument, since the destination is always `archive`.
...
```
$ ishi archive my-project
Summary for my-project? [My Project]
Moved projects/my-project to archive/projects/my-project
```

`ishi init` keeps `.vscode/settings.json` (`files.exclude`/`search.exclude`) and `.zed/settings.json` (`file_scan_exclude`) up to date with the configured archive folder name, and keeps a `CLAUDE.md` instruction not to read the archive folder unless explicitly asked or there's a strong reason to — both set up once, at init time, rather than on every archiving move.

### `unarchive`

```
ishi unarchive <OriginCategory>/<name>
```

Restores an archived item to the category it was archived from — sugar for `ishi move <OriginCategory>/<name> <origin-as-target>`, so un-archiving never requires spelling out a destination the qualified name already encodes. `<OriginCategory>/<name>` is the qualified name `ishi list archive` shows for the item.
...
$ ishi unarchive Projects/my-project
Moved archive/projects/my-project to projects/my-project
```
...
```
ishi list <inbox|project|area|resource|archive> [filter]
```
...
- **Title** is inferred the same way `ishi new`'s editor capture infers a filename: skip a leading frontmatter block, then take the first Markdown heading's text. If no heading is found, Title falls back to repeating Name.
- **Updated** is how long ago the item was last modified, in days (`today`, `1 day ago`, `12 days ago`, ...) — the same convention `ishi review`'s transcript uses.
...
modification-time signal `list` uses; "Reviewed" reflects `ishi review`, which
...
```
ishi config [init | edit] [-g | --global]
```

With no arguments, prints the effective config — [defaults](#configuration) layered with any `~/.ishi.toml` and `./.ishi.toml` overrides — annotating each setting with where it came from. `ishi config init` writes a `.ishi.toml` populated with the defaults, ready to customize; `ishi config edit` opens it in `$EDITOR`. Both target the local `./.ishi.toml` by default; pass `-g`/`--global` to target `~/.ishi.toml` instead.
...
```
ishi completions <bash|zsh|fish|powershell>
```

Prints a small registration snippet to stdout — shell glue that calls back into `ishi` (via `$PATH`) whenever you press tab, rather than a fixed script baked with today's commands. That means completions always match whichever `ishi` is currently installed, with nothing to regenerate after an upgrade — and it's how item names get completed: `ishi move`, `ishi archive`, and `ishi unarchive`'s `<item>`/`<name>` argument tab-completes from the current directory's actual PARA system (bare names for `move`/`archive`, the `<OriginCategory>/<name>` qualified form for `unarchive`), read fresh on every request. Outside a PARA system, item-name completion just offers nothing rather than erroring.
...
1. built-in defaults
2. `~/.ishi.toml` (user-level)
3. `./.ishi.toml` (local to the current PARA system)

So a template you like everywhere can live in `~/.ishi.toml`, while a project-specific folder rename goes in `./.ishi.toml` without needing to repeat the rest of your personal config. `ishi config init`/`ishi config edit` operate on `./.ishi.toml` by default and `~/.ishi.toml` with `-g`; see [`config`](#config) above.
...
Every category has a template: `note` is used for Inbox captures and `--resource` notes, `daily` for `ishi daily`, and `project`/`area` for the `index.md` scaffolded by `ishi new --project`/`--area`. Templates are plain text, filled in when the note is created, with these placeholders:

| Placeholder  | Renders to                                                                                                                                                                                                                                                                                                                                                                            |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `{{date}}`   | Today's date (`2026-07-02`)                                                                                                                                                                                                                                                                                                                                                           |
| `{{time}}`   | The current time (`14:32`)                                                                                                                                                                                                                                                                                                                                                            |
| `{{title}}`  | The note's title. Filled in from the given `filename` when creating a note non-interactively. Left empty when the template is used to pre-populate `$EDITOR` for an editor capture (`ishi new`, or `--project`/`--area`/`--resource` with no `filename`) — the title isn't known yet, since that's what the editor capture infers                                                       |
| `{{cursor}}` | Not rendered as text — marks the line where `$EDITOR`'s cursor should start, for the editor-capture paths above. Ishi opens `$EDITOR` with a `+<line>` argument pointing at that line, the convention understood by vi/vim/neovim, nano, and `emacs -nw`; editors that don't support it just open at the top of the file. Renders as an empty string outside the editor-capture paths |
...
`ishi config init` writes the defaults above as a starting point.

When an editor capture is saved, Ishi infers the title (and from it, the suggested filename) by skipping a leading frontmatter block, if present, then looking for the first Markdown heading line (any `#` level) in what follows; if no heading is found, it falls back to the first non-blank line after the frontmatter; if that's also missing, it falls back to a timestamp-based name.

### Schema and autocomplete

`ishi config init` (and `ishi config edit`, if no config exists yet) writes a [`#:schema`](https://taplo.tamasfe.dev/configuration/directives.html) directive as the first line of `.ishi.toml`, pointing at a JSON Schema that describes the `folders`, `defaults`, and `templates` keys:

```toml
#:schema ./.ishi.schema.json
...
Editors with Taplo-based TOML support — notably VS Code's [Even Better TOML](https://marketplace.visualstudio.com/items?itemName=tamasfe.even-better-toml) extension — read that directive to offer autocomplete and inline validation for `.ishi.toml`, with no extra setup. Ishi writes the referenced schema file alongside `.ishi.toml` so the reference resolves locally, without a network fetch. This is a Taplo-specific convention, not a universal TOML standard, so editors without Taplo support won't do anything with the directive beyond treating it as a comment.

## Upgrading from `tick`/`tk`

This project was previously named `tick`, with a `tk` binary. If you have an
existing `tk` install and `.tick.toml`/`.tick.schema.json` files from before
the rename, they aren't picked up automatically: install the new `ishi`
binary (see [Installation](#installation)) and rename `.tick.toml` to
`.ishi.toml` (and `.tick.schema.json` to `.ishi.schema.json`, if present) in
each PARA system, plus `~/.tick.toml` if you had a user-level config. There's
