...
fn config_init_creates_local_file_via_real_dispatch() {
    ...
    let output = common::ishi(&["config", "init"], dir.path(), None, None);

    assert!(output.status.success());
    assert_eq!(common::stdout(&output), "Created ./.ishi.toml\n");
    assert!(dir.path().join(".ishi.toml").is_file());
}
...
fn config_init_writes_schema_file_via_real_dispatch() {
    ...
    assert!(output.status.success());
    let toml_contents = std::fs::read_to_string(dir.path().join(".ishi.toml")).unwrap();
    assert!(toml_contents.starts_with("#:schema ./.ishi.schema.json"));
    assert!(dir.path().join(".ishi.schema.json").is_file());
}

#[test]
fn config_init_refuses_when_local_file_already_exists() {
    let dir = tempdir().unwrap();
    std::fs::write(dir.path().join(".ishi.toml"), "custom content").unwrap();
    ...
    assert_eq!(
        std::fs::read_to_string(dir.path().join(".ishi.toml")).unwrap(),
        "custom content"
    );
}
...
fn config_init_global_succeeds_when_local_exists_and_leaves_it_untouched() {
    ...
    std::fs::write(cwd.path().join(".ishi.toml"), "custom content").unwrap();

    let output = common::ishi_with_home(&["config", "init", "-g"], cwd.path(), home.path());

    assert!(output.status.success());
    assert_eq!(common::stdout(&output), "Created ~/.ishi.toml\n");
    assert!(home.path().join(".ishi.toml").is_file());
    assert_eq!(
        std::fs::read_to_string(cwd.path().join(".ishi.toml")).unwrap(),
        "custom content"
    );
}
...
fn config_init_global_refuses_when_global_file_already_exists() {
    let cwd = tempdir().unwrap();
    let home = tempdir().unwrap();
    std::fs::write(home.path().join(".ishi.toml"), "custom content").unwrap();
    ...
    assert!(common::stderr(&output).contains("already exists"));
    assert_eq!(
        std::fs::read_to_string(home.path().join(".ishi.toml")).unwrap(),
        "custom content"
    );
}
...
fn config_edit_opens_existing_local_file_via_real_dispatch() {
    ...
    let output = common::ishi(&["config", "edit"], dir.path(), Some(&editor), None);
...
fn config_edit_global_writes_schema_file_when_missing() {
    ...
    let output = common::ishi_with_home_and_editor(
        &["config", "edit", "-g"],
        cwd.path(),
        home.path(),
        &editor,
    ...
    assert!(output.status.success());
    assert!(home.path().join(".ishi.schema.json").is_file());
}
